...
@register_loader('pinyingpt-concat')
class PinyinGPT2ConcatLoader(PinyinGPT2Loader):

    def context_parsing(self, key, len_list):
        n_special_tokens = 4 if self.config.concat_mode == 'segmented' else 2
        opts = self.config
        ...
        if all([sum(len_) == 1 for len_ in len_list]):
            ...
            return (key, (0, 0), (0, 1)), 2 + n_special_tokens
        ...
        if opts.max_prefix_len > 0:
            ...
            # new id and len
            return (key, (start, mid), (pivot, target_len)), opts.max_txt_len + n_special_tokens - pre_context_len
        else:
            pivot = 0
            target_len = len(len_)
            while sum(len_[pivot:pivot + target_len]) * 2 > opts.max_txt_len:
                target_len -= 1
            return (key, (mid, mid), (pivot, target_len)), sum(
                len_[pivot:pivot + target_len]) * 2 + n_special_tokens
