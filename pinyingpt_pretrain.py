...
from transformers4ime.models.pinyingpt import PinyinGPTConfig
from transformers4ime.models.pinyingpt.modeling_pinyingpt import PinyinGPTConcatLMHeadModel
...
def main():
    ...
    config = PinyinGPTConfig.from_pretrained(model_args.model_name_or_path)
    logger.info("Model configurations %s", config)
    tokenizer = BertTokenizer.from_pretrained(model_args.model_name_or_path,
                                              additional_special_tokens=additional_special_tokens)
    model = PinyinGPTConcatLMHeadModel.from_pretrained(model_args.model_name_or_path, config=config)
    ...
    assert len(train_data.modalities) == 1 and 'text_pinyin' in train_data.modalities
